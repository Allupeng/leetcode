...
class Solution:
    def finalPrices(self, prices: List[int]) -> List[int]:
        # convert to "find next smaller element" problems
        discount = [0] * len(prices)
        stack = []
        for i, price in enumerate(prices):
            while stack and prices[stack[-1]] >= price:
                prev_index = stack.pop()
                discount[prev_index] = price
            stack.append(i)
        result = [prices[i] for i in range(len(prices))]
        for i in range(len(discount)):
            result[i] = result[i] - discount[i]
        return result
# @lc code=end
...
class TestFinalPrices(unittest.TestCase):
    ...
    def test_case1(self):
        prices = [8,4,6,2,3]
        expected = [4,2,4,2,3]
        self.assertListEqual(self.solution.finalPrices(prices), expected)
